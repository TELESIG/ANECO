...
#' @param whereparallel A character string indicating the level at which parallelization should be applied. If set to 'files', the function will process each audio file in parallel. If set to 'minutes', parallelization will be applied across the one-minute segments within each file.
...
acoustic_indices <- function(dir = NULL, calibparam = "SM4", sel.ind = "all", exclude = "none", noise.ind = TRUE, bioband = c(1000, 11000), noiseband = c(0, 1000), channel = "left", prefix.format = "SSGG", wl = 512, ncores = NULL, save.file = TRUE, add.prefix = "XXXX", whereparallel = "files"){
...
# ---------------------------------
  ...
  ap <- future.apply::future_lapply(X = filenumber, FUN = function(xi){
    ...
    if(whereparallel == "files") {
      progress_files(message = paste0("Processing file ", xi, " of ", length(filenumber), "  "))
    }
